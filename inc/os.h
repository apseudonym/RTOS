...
#define TIME_1MS  SysCtlClockGet()/1000  
#define TIME_2MS  2*TIME_1MS 
#define PERIODIC_FUNCTION_RATE 10000 

struct tcb{
  long *sp;          // pointer to stack, valid for threads not running
  struct tcb* next;  // active linked-list pointers to next TCB			
  struct tcb* prev;
  struct tcb* blockedNext;  // blocked linked-list pointers to next TCB
  struct tcb* blockedPrev; 
  short status;  // blocked status (can implement sleep into it...)
  short sleep;   // sleep status
  long id;       // thread id
  char *name;    // 0 if active, 1 if sleeping
...
struct  Sema4{
  ...
  struct tcb* tail;
	//problem:
	//  when multiple semaphores use the same node, their 
	//  blockedNext and blockedPrev pointers will fuck up 
	//  because the node is already part of another LL
	//
	//...but
	//  no more than one semaphore can hold onto a thread 
	//  (because a blocked thread can't call for a new semaphore)
	//  so it should be ok!
	//
...
//unsigned long OS_Fifo_Get(unsigned long *datapt);
...
int OS_AddPeriodicThread(void(*task_in)(void), unsigned long period_in, unsigned long priority_in);
