...
#define PERIOD 5     // 2kHz sampling period in system time units
...
void Interpreter(void){
  for(;;){
    //UART_Message("NumSamples ",NumSamples);
 	//UART_Message("NumCreated ",NumCreated);
  	//UART_Message("PIDWork    ",PIDWork);
  	//UART_Message("Actuator   ",Actuator);
  	//UART_Message("Jitter     ",MaxJitter-MinJitter);
    //UART_Message("\f",0);
  }
}
//--------------end of Task 5-----------------------------
...
int finalmain(void){        // lab 3 real main
...
//*******attach background tasks***********
  ...
  OS_AddDownButtonTask(&DownPush,3);
  OS_AddPeriodicThread(&DAS,PERIOD,1); // 2 kHz real time sampling

  NumCreated = 0 ;
// create initial foreground threads
  NumCreated += OS_AddThread(&Interpreter,256,2); 
  NumCreated += OS_AddThread(&Consumer,256,1); 
  NumCreated += OS_AddThread(&PID,256,3); 
  //NumCreated += OS_AddThread(&UART_Output_Thread,256,2); 
  NumCreated += OS_AddThread(&Message_Output_Thread,256,2); 
  OS_Launch(TIMESLICE); // doesn't return, interrupts enabled in here
  return 0;             // this never executes
}

void PrintADC0(unsigned short in){
    UART_Message("ADC0 ",in);
    oLED_Message(TOP,0,"ADC0 ",in);
}
void PrintADC1(unsigned short in){
    UART_Message("ADC1 ",in);
    oLED_Message(TOP,1,"ADC1 ",in);
}
void PrintADC2(unsigned short in){
    UART_Message("ADC2 ",in);
    oLED_Message(TOP,2,"ADC2 ",in);
}
void PrintADC3(unsigned short in){
    UART_Message("ADC3 ",in);
    oLED_Message(TOP,3,"ADC3 ",in);
}

void PrintADCs(void){
    for(;;){    
        oLED_Message(BOTTOM,0,"ADC0 ",ADC_In(0));
        oLED_Message(BOTTOM,1,"ADC1 ",ADC_In(1));
        oLED_Message(BOTTOM,2,"ADC2 ",ADC_In(2));
        oLED_Message(BOTTOM,3,"ADC3 ",ADC_In(3));
        OS_Sleep(100);
    }        
}
...
int adcmain(void){        // ADC Test main
    ...
    ADC_Collect(0,0,PrintADC0);
    //ADC_Collect(1,0,PrintADC1);
    //ADC_Collect(2,0,PrintADC2);
    //ADC_Collect(3,0,PrintADC3);
...
void Thread2c(void){
  ...
  OS_AddPeriodicThread(&BackgroundThread1c,PERIOD/2,0); 
  for(;;){
    OS_Wait(&Readyc);
...
int Testmain5(void){       // Testmain5
  ...
  OS_AddPeriodicThread(&TaskA,PERIOD*2,0);           // 1 ms, higher priority
  OS_AddPeriodicThread(&TaskB,PERIOD*4,1);         // 2 ms, lower priority
