...
/*
 * From Program 4.4.
 * TCBs for up to 3 threads; stack is 400 bytes.
 */
struct tcb{
  long *sp;						// pointer to stack, valid for threads not running
  struct tcb* next;		// linked-list pointer to next TCB				// thread name
	short sleep;
	long id;						// thread id
...
void checkSleep(void){
	...
	}while(RunPt->sleep != 0 || RunPt->priority != 0);
...
void GPIOPortE_Handler(void){
	...
	GPIOPinIntClear(GPIO_PORTF_BASE, GPIOPinIntStatus(GPIO_PORTE_BASE, false));
	//IntMasterEnable();
}
